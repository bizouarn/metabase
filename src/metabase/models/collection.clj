...
(t2/define-before-delete :model/Collection
  ...
  ;; Delete all the children of this Collection. But first! Move any subcollections that *weren't* trashed WITH us up
  ;; a level. TODO: they need to be moved up as many levels as it takes, if we're deleting subcollections...
  (t2/query-one
   {:update :collection
    :set {:location [:replace :location (children-location collection) "/"]}
    :where [:and
            [:like :location (str (children-location collection) "%")]
            [:not= :trash_operation_id (:trash_operation_id collection)]]})
  (t2/delete! :model/Collection
              :location (children-location collection)
              :trash_operation_id [:= (:trash_operation_id collection)])
  ;; TODO: When deleting a collection, we should leave things that weren't trashed with the collection. But we can't,
  ;; because permissions come from the collection.
